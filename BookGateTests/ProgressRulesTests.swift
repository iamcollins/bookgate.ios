...
final class ProgressRulesTests: XCTestCase {
    ...
    func testConsecutiveNightsAdvanceTheStreak() {
        let p = ProgressStore()
        p.recordNight(minutes: 10, elapsed: 600, now: day(-2))
        p.recordNight(minutes: 10, elapsed: 600, now: day(-1))
        p.recordNight(minutes: 10, elapsed: 600, now: day(0))
        XCTAssertEqual(p.currentStreak, 3)
    }

    /// Twice in one night is still one night.
    func testTwoSessionsInOneNightCountOnce() {
        let p = ProgressStore()
        p.recordNight(minutes: 10, elapsed: 600, now: day(-1))
        p.recordNight(minutes: 10, elapsed: 600, now: day(0))
        p.recordNight(minutes: 20, elapsed: 1200, now: day(0))
        XCTAssertEqual(p.currentStreak, 2)
        XCTAssertEqual(p.minutes(on: day(0)), 20, "the longer session of the night should stand")
    }

    /// One missed night is forgiven — the rule Settings states.
    func testOneMissedNightIsForgivenByTheRestDay() {
        let p = ProgressStore()
        p.recordNight(minutes: 10, elapsed: 600, now: day(-3))
        p.recordNight(minutes: 10, elapsed: 600, now: day(-2))
        // day(-1) missed
        p.recordNight(minutes: 10, elapsed: 600, now: day(0))
    ...
    func testASecondRestDayInTheSameWeekIsNotGranted() {
        let p = ProgressStore()
        p.recordNight(minutes: 10, elapsed: 600, now: day(-6))
        p.recordNight(minutes: 10, elapsed: 600, now: day(-4))   // first miss, forgiven
        XCTAssertEqual(p.currentStreak, 2)
        p.recordNight(minutes: 10, elapsed: 600, now: day(-2))   // second miss, same week
    ...
    func testTheRestDayReturnsAfterSevenDays() {
        let p = ProgressStore()
        p.recordNight(minutes: 10, elapsed: 600, now: day(-20))
        p.recordNight(minutes: 10, elapsed: 600, now: day(-18))  // forgiven
    ...
    func testTwoMissedNightsBreakTheStreak() {
        let p = ProgressStore()
        p.recordNight(minutes: 10, elapsed: 600, now: day(-4))
        p.recordNight(minutes: 10, elapsed: 600, now: day(-3))
